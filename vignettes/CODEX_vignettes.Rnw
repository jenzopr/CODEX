...
Installation inforation is available \href{https://sites.google.com/a/cornell.edu/yuchaojiang/home/research/codex}{here}. The online Q\&A forum is available \href{https://groups.google.com/d/forum/codex_wes_cnv}{here}. If you've any questions regarding the software, please don't email us at \href{mailto:codex_wes_cnv@googlegroups.com}{codex\_wes\_cnv@googlegroups.com} and/or \\
\href{mailto:yuchaoj@wharton.upenn.edu}{yuchaoj@wharton.upenn.edu}.
...
<<bambedObj2, eval=FALSE>>=
save(bambedObj, file = paste(projectname, '_', chr, '_bambedObj', '.rda', sep=''),
     compress='xz')
@
...
<<coverageObj2, eval=FALSE>>=
save(coverageObj, file = paste(projectname, '_', chr, '_coverageObj', '.rda', sep=''),
     compress='xz')
@
...
<<gcmapp2, eval=FALSE>>=
save(gc, file = paste(projectname, '_', chr, '_gc', '.rda', sep=''),
     compress='xz')
save(mapp, file = paste(projectname, '_', chr, '_mapp', '.rda', sep=''),
     compress='xz')
@
...
save(qcObj, file = paste(projectname, '_', chr, '_qcObj', '.rda', sep=''),
     compress='xz')
@
...
<<normObj2, eval=FALSE>>=
save(normObj, file = paste(projectname, '_', chr, '_normObj', '.rda', sep='')
     ,compress='xz')
@
...
length\_kb (CNV length in kb), st\_exon (the first exon in the cnv, integer
value numbered in qcObj\$qcmat), ed\_exon (the last exon in the cnv,
integer value numbered in qcObj\$qcmat), raw\_cov (raw coverage), norm\_cov
(normalized coverage), copy\_no (copy number estimate), lratio (likelihood
ratio of CNV event versus copy neutral event), mBIC (modified BIC value,
used to determine the stop point of segmentation), pvalue (p-values computed
by the Wilk's theorem from the likelihood ratio test).
\\
...
finalcall <- segment(Y_qc, Yhat, optK = optK, K = K, sampname_qc,
    ref_qc, ref, chr, lmax = 200, mode = "integer")
finalcall
<<segment2, eval=FALSE>>=
write.table(finalcall, file = paste(projectname, '_', chr, '_', optK, '_CODEX_frac.txt',
